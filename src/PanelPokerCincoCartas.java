...
public class PanelPokerCincoCartas extends JPanel {
    ...
    private void crearPanelMesa() {
        ...
        try {
            ...
            ImageIcon logoIcon = new ImageIcon("G:\\4toSemestre\\POO\\Poker-Proyecto-Final\\src\\recursos\\fondo_poker.png");
            Image imagenRedimensionada = logoIcon.getImage().getScaledInstance(200, 200, Image.SCALE_SMOOTH);
    ...
    private void crearPanelDescarte() {
        ...
        panelDescarte.setPreferredSize(new Dimension(180, 300));
    ...
    private void actualizarEstadoJuego() {
        ...
        for (int i = 0; i < panelesJugadores.size(); i++) {
            boolean mostrarCartas = (i == estado.turnoActualIndice);
    ...
    private class PanelJugador extends JPanel {
        ...
        public void actualizar(Jugador.Info info, boolean mostrarCartas) {
            ...
            // Actualizar cartas
            if (info.tieneCartas && info.cartas != null && mostrarCartas) {
                // Mostrar cartas del jugador activo
                for (int i = 0; i < cartasLabels.size(); i++) {
                    if (i < info.cartas.size()) {
                        cartasLabels.get(i).setText(info.cartas.get(i).toString());
                        cartasLabels.get(i).setBackground(Color.WHITE);
                    } else {
                ...
                // Ocultar cartas para jugadores no activos
                for (JLabel carta : cartasLabels) {
                    if (info.tieneCartas) {
                        carta.setText("?");
                        carta.setBackground(new Color(200, 30, 30));
                    } else {
                        carta.setText("");
                        carta.setBackground(new Color(200, 200, 200));
                    }
                }
            }
